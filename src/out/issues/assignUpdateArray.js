...
const issue = {
    ...
    detector: (files) => {
        let instances = [];
        if (!!files.ast) {
            for (const node of (0, utils_1.findAll)('Assignment', files.ast)) {
                if (node.operator === '=' &&
                ...
                //   node.leftHandSide.typeDescriptions.typeString?.includes('[] memory'))
                    ...
                    if (node.rightHandSide.rightExpression.nodeType === 'IndexAccess') {
                        if ((!array ||
                            ...
                            instances.push((0, utils_2.instanceFromSRC)(files, node.src));
                        }
                    }
...
//# sourceMappingURL=assignUpdateArray.js.map